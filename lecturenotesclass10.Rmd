...
```
Using set.seed(100) would make it the same every time. 
...
Very close. Red is slightly below, blue is slightly above, and white is pretty much spot-on, and as you increase the sample size, these probabilities will get closer and closer to the "true" probabilities. 
...
I would guess that this would be somewhat unlikely. 
...
```
It is false 59.88% of the time, and true 40.12% of the time, so my hypothesis that it would be somewhat unlikely was true. The match still happens often but it is more unlikely than not.
...
I would guess that it would be more likely for the absolute value of the difference between dice roll 1 and dice roll 2 to be equal to 4. 
